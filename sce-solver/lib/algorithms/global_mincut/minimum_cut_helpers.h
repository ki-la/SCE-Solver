                ...
                if (new_graph->getMinDegree() < previous_mincut) {
                    ...
                    for (NodeID idx : graphs[0]->nodes()) {
                        ...
                        if (coarseID == minimum_index) {
                            //std::cout << "setNodeInCut: " << idx << "\n";
                            graphs[0]->setNodeInCut(idx, true);
                        } else {
                            //std::cout << "setNodeInCut: " << idx << "false \n";
                            graphs[0]->setNodeInCut(idx, false);
                        }
                    }
                }
...
#pragma omp parallel for
        ...
        if (configuration::getConfig()->save_cut) {
            ...
            if (cut_logs) {
                ...
                std::sort(b.begin(), b.end());
                [[maybe_unused]] bool verbose = configuration::getConfig()->verbose;
                [[maybe_unused]] NodeID id0 = b.end() - std::upper_bound(b.begin(), b.end(), 0);
                [[maybe_unused]] NodeID id1 = b.end() - std::upper_bound(b.begin(), b.end(), 1);
    ...
    static void printLogs([[maybe_unused]] const std::vector<NodeID>& b) {
